...
export class OrderRequestComponent implements OnInit {
  ...
  // Existing files loaded from API
  existingFiles: any[] = [];
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (params['rowIndex']) {
        ...
        if (!isNaN(rowIndex) && rowIndex > 0) {
          ...
          // Fetch the complete order request details from the API
          this.fetchOrderRequestFromApi(rowIndex);
  ...
  /**
   * Fetch complete order request from API by rowIndex
   * This method calls the new GetOrderRequestByRowIndex API endpoint
   */
  private fetchOrderRequestFromApi(rowIndex: number): void {
    this.reportService.getOrderRequestByRowIndex(rowIndex).subscribe({
      next: (orderRequest: any) => {
        if (!orderRequest) {
          console.warn('Order request not found for rowIndex:', rowIndex);
          return;
        }

        // Set the form values from the API response
        const orderDate = orderRequest.orderDate ? this.formatDateForInput(orderRequest.orderDate) : null;
        const arriveDate = orderRequest.arriveDate ? this.formatDateForInput(orderRequest.arriveDate) : null;

        this.orderRequestForm.patchValue({
          rowIndex: orderRequest.rowIndex || rowIndex,
          orderType: this.normalizeOrderType(orderRequest.orderType || ''),
          requester: (orderRequest.requester || '').trim(),
          dcgPartNo: (orderRequest.dcgPartNo || '').trim(),
          manufPartNo: (orderRequest.manufPartNo || '').trim(),
          qtyNeeded: orderRequest.qtyNeeded || null,
          vendor: (orderRequest.vendor || '').trim(),
          poNumber: (orderRequest.poNumber || '').trim(),
          orderDate: orderDate,
          arriveDate: arriveDate,
          status: (orderRequest.status || 'NEW').trim(),
          notes: (orderRequest.notes || '').trim()
        });

        // Set the auto-generated ID
        this.setAutoGeneratedIdFromParams(orderRequest, rowIndex);

        // Load existing files for this order request
        this.loadExistingFiles(rowIndex);
      },
      error: (error) => {
        console.error('Error fetching order request from API:', error);
        // Continue without API data - form will remain empty or with defaults
      }
    });
  }

  /**
   * Load existing files for an order request from the API
   */
  loadExistingFiles(rowIndex: number): void {
    this.isLoadingFiles = true;
    this.fileDeleteError = null;
    this.orderRequestService.getOrderRequestFiles(rowIndex).subscribe({
      next: (files: any) => {
        this.existingFiles = Array.isArray(files) ? files : (files?.files || files?.Files || []);
        this.isLoadingFiles = false;
      },
      error: (error) => {
        console.error('Error loading files for order request:', error);
        this.existingFiles = [];
        this.isLoadingFiles = false;
      }
    });
  ...
  /**
   * Delete an existing file from the order request
   */
  deleteExistingFile(fileName: string): void {
    const rowIndex = this.orderRequestForm.get('rowIndex')?.value;
    if (!rowIndex) return;
    this.fileDeleteError = null;
    this.orderRequestService.deleteOrderRequestFile(rowIndex, fileName).subscribe({
      next: () => {
        this.loadExistingFiles(rowIndex);
      },
      error: (error) => {
        console.error('Error deleting file:', error);
        this.fileDeleteError = error.error?.message || 'Failed to delete file. Please try again.';
      }
    });
  }
  ...
  getFileIcon(fileName: string): string {
    ...
    const iconMap: { [key: string]: string } = {
      ...
      bmp: 'bi-file-earmark-image text-warning',
  ...
  private loadOrderRequestDetailsFromApi(rowIndex: number): void {
    this.orderRequestService.getPartsTestList(rowIndex, 'OrderRequest').subscribe({
      next: (data: PartsTestListResponse) => {
        ...
        this.orderRequestForm.patchValue({
          orderType: this.normalizeOrderType(row.OrderType || row.orderType || ''),
          ...
          orderDate: this.formatDateForInput(row.OrderDate) || this.orderRequestForm.get('orderDate')?.value,
          arriveDate: this.formatDateForInput(row.ArriveDate) || this.orderRequestForm.get('arriveDate')?.value,
  ...
  saveOrderRequest(): void {
    ...
    const orderRequest: SaveUpdateOrderRequestRequest = {
      ...
      orderType: this.normalizeOrderType(formValue.orderType) || null,
    ...
    if (this.selectedFiles.length > 0) {
      ...
      this.orderRequestService.saveUpdateOrderRequestWithFiles(orderRequest, this.selectedFiles).subscribe({
        next: (response: any) => {
          ...
          const savedRowIndex = response.rowIndex || orderRequest.rowIndex;
          if (savedRowIndex > 0) { this.loadExistingFiles(savedRowIndex); }
      ...
      this.orderRequestService.saveUpdateOrderRequest(orderRequest).subscribe({
        next: (response: any) => {
          ...
          // Refresh existing files list
          const savedRowIndex2 = response.rowIndex || orderRequest.rowIndex;
          if (savedRowIndex2 > 0) { this.loadExistingFiles(savedRowIndex2); }
  ...
  /**
   * Format date for HTML date input, filtering out invalid default dates
   */
  private formatDateForInput(date: Date | string | null | undefined): string | null {
    ...
    try {
      ...
      // Don't format dates from 1900 or earlier
  ...
  private normalizeOrderType(value: string | null | undefined): string {
