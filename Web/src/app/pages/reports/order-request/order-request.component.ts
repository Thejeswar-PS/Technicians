...
export class OrderRequestComponent implements OnInit {
  ...
  /**
   * Generate auto ID in format YYYY-MM-DD## (e.g., 2026-01-1261)
   */
  generateAutoId(): void {
    const now = new Date();
    ...
    this.orderRequestService.getMaxOrderRequestRowIndex().subscribe({
      next: (maxRowIndex: number) => {
        const nextSequence = (maxRowIndex + 1).toString().padStart(2, '0');
        
        this.autoGeneratedId = {
          year: now.getFullYear().toString(),
          month: (now.getMonth() + 1).toString().padStart(2, '0'),
          day: now.getDate().toString().padStart(2, '0'), // Two digit day with leading zero
          id: nextSequence // Database-driven sequence number (2 digits)
        };
      ...
      error: (error) => {
        ...
        const fallbackSequence = Date.now().toString().slice(-2).padStart(2, '0');
